//Joey Phan - Wednesday 5PM, Lab 6, Potika
...
//function definition: recurively reverses the columns of an image
void MirrorCols(IMAGE *image, unsigned row, unsigned min, unsigned max)
	{
	// To be completed by student ...
	if(min<=max){ 
		PIXEL temp = image->pxlrow[row][min];
		image->pxlrow[row][min] = image->pxlrow[row][max];
		image->pxlrow[row][max] = temp; //swaps each pixel	
		MirrorCols(image, row, min+1, max-1);//increments to the next set of pixels in the row
	}else
		return;
	}
	
//function definition: recurively resevses the rows of an image
void MirrorRows(IMAGE *image, unsigned min, unsigned max)
	{
	// To be completed by student ...
		if (min < max){ 
		PIXEL *temp = image->pxlrow[min];
		image->pxlrow[min] = image->pxlrow[max];	
		image->pxlrow[max]= temp; //swaps row 
		MirrorRows(image, min+1, max-1);//increment to the next set of rows
		}else
			return;
	}
